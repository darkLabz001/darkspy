...
#
# DarkSpy is distributed in the hope that it will be useful,
...
class netcraft:
	...
	def search(self):
		...
		try:
			url = "https://searchdns.netcraft.com/?restriction=site+contains&host=%s&lookup=wait..&position=limited"%(self.target)
			resp = self.con.urllib(url,None)
			if resp:
				sites = re.findall(r'url=\S+"',resp,re.I)
				if sites:
